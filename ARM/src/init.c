...
#define MSI0_D0_PORTD_FER   (1 << BITP_PORT_DATA_PX15)
#define MSI0_D1_PORTE_FER   (1 << BITP_PORT_DATA_PX1)
#define MSI0_D2_PORTE_FER   (1 << BITP_PORT_DATA_PX6)
#define MSI0_D3_PORTE_FER   (1 << BITP_PORT_DATA_PX8)
#define MSI0_CMD_PORTG_FER  (1 << BITP_PORT_DATA_PX1)
#define MSI0_CLK_PORTE_FER  (1 << BITP_PORT_DATA_PX9)
#define MSI0_CD_PORTB_FER   (1 << BITP_PORT_DATA_PX15)

/* MSI0 GPIO MUX bit positions (two bits per MUX entry, all mux 0) */
#define MSI0_D0_PORTD_MUX   (1 << (BITP_PORT_DATA_PX15 << 1))
#define MSI0_D1_PORTE_MUX   (1 << (BITP_PORT_DATA_PX1 << 1))
#define MSI0_D2_PORTE_MUX   (2 << (BITP_PORT_DATA_PX6 << 1))
#define MSI0_D3_PORTE_MUX   (1 << (BITP_PORT_DATA_PX8 << 1))
#define MSI0_CMD_PORTG_MUX  (2 << (BITP_PORT_DATA_PX1 << 1))
#define MSI0_CLK_PORTE_MUX  (1 << (BITP_PORT_DATA_PX9 << 1))
#define MSI0_CD_PORTB_MUX   (2 << (BITP_PORT_DATA_PX15 << 1))
 ...
 **********************************************************************/
typedef struct ETH_CB_INFO {
    ADI_CALLBACK cb;
    uint32_t event;
    void *usr;
} ETH_CB_INFO;

ETH_CB_INFO ethCBInfo[2] = { 0 };

void eth_phy_cb_init(uint8_t eth, ADI_CALLBACK cb, uint32_t event, void *usr)
{
    ETH_CB_INFO *ecbi = &ethCBInfo[eth];
    ecbi->cb = cb;
    ecbi->event = event;
    ecbi->usr = usr;
}

static void eth_phy_int(ADI_GPIO_PIN_INTERRUPT const pint,
    uint32_t const pins, void *usr)
{
    ETH_CB_INFO *ecbi = (ETH_CB_INFO *)usr;
    if (ecbi->cb) {
        ecbi->cb(ecbi->usr, ecbi->event, NULL);
    }
}

/*
 * The EMAC0 PHY interrupt is only positive edge sensitve and the
 * DP83867 cannot drive a positive edge interrupt.  Use the Port D.12
 * GPIO interrupt instead.
 */
static void eth_emac0_init(APP_CONTEXT *context)
{
    ...
    *pREG_PADS0_PCFG0 |= BITM_PADS_PCFG0_EMACRESET;
    ...
    /* PHY IRQ configuration.  PD.12, edge sensitive.
     * PINT3 maps to SEC IRQ 68 / GIC IRQ 100
     * Rev D SoM carrier board has rising edge PHY interrupt others
     * have falling edge interrupt */
    ADI_GPIO_SENSE edge =
        (context->SoMCRRVersion == SOMCRR_REV_D) ?
            ADI_GPIO_SENSE_RISING_EDGE : ADI_GPIO_SENSE_FALLING_EDGE;
    result = adi_gpio_PinInterruptAssignment(
...
{
    int version = SOM_REV_D;
    uint8_t rx[1];
    uint8_t tx[1] = { 0x01 };

    /*
     * The SOM board rev can be detected by the soft-switch 
     * attached to the TWI2 port. Each soft-switch between 
     * the SOMs has a different TWI address (0x34U for rev E and 0x20U for 
     * rev D and lower) so we can read a known register value from each 
     * to determine the variant. 
     * 
     * Default to Rev D
     */

    if(twi_writeRead(context->softSwitchHandle, 0x34U, tx, sizeof(tx), rx, sizeof(rx)) == TWI_SIMPLE_SUCCESS)
    {
        version = SOM_REV_E;
    }
