 ...
 * Defines
 **********************************************************************/
#ifdef SDCARD_USE_EMMC
#define CARD_TYPE SDCARD_CARD_TYPE_EMMC
#define VOL_NAME  EMMC_VOL_NAME
#else
#define CARD_TYPE SDCARD_CARD_TYPE_SD
#define VOL_NAME  SDCARD_VOL_NAME
#endif
...
static void sdcardCheck(APP_CONTEXT *context)
{
    ...
    FRESULT fatResult;
    char *cardName = context->cardName;
        ...
        if (sd) {
            ...
            if (sdResult == SDCARD_SIMPLE_SUCCESS) {
                ...
                if (sdResult == SDCARD_SIMPLE_SUCCESS) {
                    ...
                    fatResult = f_mount(fs, VOL_NAME, 1);
                    if (fatResult == FR_OK) {
...
void ledFlashOff(TimerHandle_t xTimer)
{
    ...
    ss_set(context, SS_PIN_ID_DS1, 1);
}
...
static portTASK_FUNCTION( houseKeepingTask, pvParameters )
{
    ...
    while (1) {
        ...
        /* Flash the LED */
        ss_set(context, SS_PIN_ID_DS1, 0);
...
#ifdef SDCARD_USE_EMMC
    context->cardName = "eMMC";
#else
    context->cardName = "SDCARD";
#endif

    /* Open the eMMC/SD driver */
    sdcardResult = sdcard_open(SDCARD0, CARD_TYPE, &context->sdcardHandle);

    /* Hook the eMMC/SD card filesystem into the stdio libraries */
    if (sdcardResult == SDCARD_SIMPLE_SUCCESS) {
        device = fs_dev_fatfs_device();
        fsdResult = fs_devman_register(VOL_NAME, device, NULL);
        fsdResult = fs_devman_set_default(VOL_NAME);
    }
