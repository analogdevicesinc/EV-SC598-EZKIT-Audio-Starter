...
#define SOM_GET ss_get_sc598_som
#define CRR_SET ss_set_somcrr_ezkit
#define SOM_SET ss_set_sc598_som
...
static SS_PIN SS_PINS[] = {
    ...
    { .pinId = SS_PIN_ID_DS1, .get = SOM_GET, .set = SOM_SET},
    { .pinId = SS_PIN_ID_DS2, .get = SOM_GET, .set = SOM_SET},
    { .pinId = SS_PIN_ID_DS3, .get = SOM_GET, .set = SOM_SET},
    { .pinId = SS_PIN_ID_EMMC_EN, .get = SOM_GET, .set = SOM_SET},
    { .pinId = SS_PIN_ID_EMMC_SOM_EN, .get = SOM_GET, .set = SOM_SET},
...
void ss_init(APP_CONTEXT *context)
{
    ...
    ss_init_sc598_som(context);
#ifdef SDCARD_USE_EMMC
    ss_set(context, SS_PIN_ID_EMMC_SOM_EN, 1);
    ss_set(context, SS_PIN_ID_EMMC_EN, 0);
#endif
}
